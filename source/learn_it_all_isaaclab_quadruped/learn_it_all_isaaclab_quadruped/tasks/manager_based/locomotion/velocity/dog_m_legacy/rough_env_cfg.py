...
from learn_it_all_isaaclab_quadruped.assets.robots import DOG_M_LEGACY_CFG


@configclass
class DogMLegacyRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    def __post_init__(self):
        super().__post_init__()

        self.scene.robot = DOG_M_LEGACY_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        ...
        self.rewards.undesired_contacts.params["sensor_cfg"].body_names = ".*ankle_link"
...
@configclass
class DogMLegacyRoughEnvCfgPlay(DogMLegacyRoughEnvCfg):
    def __post_init__(self):
        super().__post_init__()
